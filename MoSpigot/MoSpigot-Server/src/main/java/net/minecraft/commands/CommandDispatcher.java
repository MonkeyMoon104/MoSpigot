...
public class CommandDispatcher {
    ...
    private static final PacketPlayOutCommands.e<CommandListenerWrapper> COMMAND_NODE_INSPECTOR = new PacketPlayOutCommands.e<CommandListenerWrapper>() {
        @Nullable
        @Override
        public MinecraftKey suggestionId(ArgumentCommandNode<CommandListenerWrapper, ?> argumentcommandnode) {
            SuggestionProvider<CommandListenerWrapper> suggestionprovider = argumentcommandnode.getCustomSuggestions();

            return suggestionprovider != null ? CompletionProviders.getName(suggestionprovider) : null;
        }

        @Override
        public boolean isExecutable(CommandNode<CommandListenerWrapper> commandnode) {
            return commandnode.getCommand() != null;
        }

        @Override
        public boolean isRestricted(CommandNode<CommandListenerWrapper> commandnode) {
            Predicate predicate = commandnode.getRequirement();
            boolean flag;

            if (predicate instanceof PermissionCheck<?> permissioncheck) {
                if (permissioncheck.requiredLevel() > 0) {
                    flag = true;
                    return flag;
                }
            }
    ...
    public CommandDispatcher(CommandDispatcher.ServerType commanddispatcher_servertype, CommandBuildContext commandbuildcontext) {
        ...
        CommandDatapack.register(this.dispatcher, commandbuildcontext);
        ...
        CommandGamerule.register(this.dispatcher, commandbuildcontext);
        ...
        VersionCommand.register(this.dispatcher, commanddispatcher_servertype != CommandDispatcher.ServerType.INTEGRATED);
        ...
        GameTestHarnessTestCommand.register(this.dispatcher, commandbuildcontext);
    ...
    public void performCommand(ParseResults<CommandListenerWrapper> parseresults, String s, String label) { // CraftBukkit
        ...
        try {
            ...
            commandlistenerwrapper.sendFailure(IChatBaseComponent.translatable("command.failed").withStyle((chatmodifier) -> {
                return chatmodifier.withHoverEvent(new ChatHoverable.e(ichatmutablecomponent));
            }));
            if (SharedConstants.IS_RUNNING_IN_IDE) {
        ...
        try {
            ...
            if (commandsyntaxexception.getInput() != null && commandsyntaxexception.getCursor() >= 0) {
                ...
                if (i < commandsyntaxexception.getInput().length()) {
                    IChatBaseComponent ichatbasecomponent = IChatBaseComponent.literal(commandsyntaxexception.getInput().substring(i)).withStyle(EnumChatFormat.RED, EnumChatFormat.UNDERLINE);
    ...
    public static void executeCommandInContext(CommandListenerWrapper commandlistenerwrapper, Consumer<ExecutionContext<CommandListenerWrapper>> consumer) {
        ...
        if (flag) {
            ...
            try {
                ExecutionContext<CommandListenerWrapper> executioncontext1 = new ExecutionContext<CommandListenerWrapper>(i, j, Profiler.get());
    ...
    public void sendCommands(EntityPlayer entityplayer) {
        if ( org.mospigot.config.MoSpigotConfig.tabComplete < 0 ) return; // Spigot
        // CraftBukkit start
    ...
    private static <S> void fillUsableCommands(CommandNode<S> commandnode, CommandNode<S> commandnode1, S s0, Map<CommandNode<S>, CommandNode<S>> map) {
        for (CommandNode<S> commandnode2 : commandnode.getChildren()) {
            if ( !org.mospigot.config.MoSpigotConfig.sendNamespaced && commandnode2.getName().contains( ":" ) ) continue; // Spigot
            if (commandnode2.canUse(s0)) {
                ...
                if (!commandnode2.getChildren().isEmpty()) {
                    fillUsableCommands(commandnode2, commandnode3, s0, map);
                }
            }
        }
    ...
    public static <T extends PermissionSource> PermissionCheck<T> hasPermission(int i) {
        return new PermissionSource.a<T>(i);
    }
