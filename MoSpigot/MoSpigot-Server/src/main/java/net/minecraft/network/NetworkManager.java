...
public class NetworkManager extends SimpleChannelInboundHandler<Packet<?>> {
        ...
        if (throwable instanceof SkipEncodeException) {
            ...
            if (this.channel.isOpen()) {
                if (throwable instanceof TimeoutException) {
                    ...
                    IChatBaseComponent ichatbasecomponent = IChatBaseComponent.translatable("disconnect.genericReason", "Internal Exception: " + String.valueOf(throwable));
                    PacketListener packetlistener = this.packetListener;
                    DisconnectionDetails disconnectiondetails;

                    if (packetlistener != null) {
                        disconnectiondetails = packetlistener.createDisconnectionInfo(ichatbasecomponent, throwable);
                    } else {
                        disconnectiondetails = new DisconnectionDetails(ichatbasecomponent);
                    }

                    if (flag) {
                        NetworkManager.LOGGER.debug("Failed to sent packet", throwable);
                        if (this.getSending() == EnumProtocolDirection.CLIENTBOUND) {
                            Packet<?> packet = (Packet<?>) (this.sendLoginDisconnect ? new PacketLoginOutDisconnect(ichatbasecomponent) : new ClientboundDisconnectPacket(ichatbasecomponent));
    ...
    private void validateListener(ProtocolInfo<?> protocolinfo, PacketListener packetlistener) {
        ...
        if (enumprotocoldirection != this.receiving) {
            ...
            if (protocolinfo.id() != enumprotocol) {
                String s1 = String.valueOf(enumprotocol);
    ...
    private <S extends ServerboundPacketListener, C extends ClientboundPacketListener> void initiateServerboundConnection(String s, int i, ProtocolInfo<S> protocolinfo, ProtocolInfo<C> protocolinfo1, C c0, ClientIntent clientintent) {
        if (protocolinfo.id() != protocolinfo1.id()) {
            ...
            this.runOnceConnected((networkmanager) -> {
                ...
                networkmanager.sendPacket(new PacketHandshakingInSetProtocol(SharedConstants.getCurrentVersion().protocolVersion(), s, i, clientintent), (ChannelFutureListener) null, true);
    ...
    private static ChannelOutboundHandler createFrameEncoder(boolean flag) {
        return (ChannelOutboundHandler) (flag ? new LocalFrameEncoder() : new PacketPrepender());
    }
    ...
    private static ChannelInboundHandler createFrameDecoder(@Nullable BandwidthDebugMonitor bandwidthdebugmonitor, boolean flag) {
        return (ChannelInboundHandler) (!flag ? new PacketSplitter(bandwidthdebugmonitor) : (bandwidthdebugmonitor != null ? new MonitoredLocalFrameDecoder(bandwidthdebugmonitor) : new LocalFrameDecoder()));
    }
