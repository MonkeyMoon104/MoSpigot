...
public class EntityCreeper extends EntityMonster {
    ...
    public void explodeCreeper() {
        ...
        if (world instanceof WorldServer worldserver) {
            float multiplier = this.isPowered() ? 6.0F : 3.0F;

            // CraftBukkit start
            ExplosionPrimeEvent event = CraftEventFactory.callExplosionPrimeEvent(this, this.explosionRadius * multiplier, false);
            if (!event.isCancelled()) {
            // CraftBukkit end
            this.dead = true;
            worldserver.explode(this, net.minecraft.world.level.Explosion.getDefaultDamageSource(this.level(), this).customCausingEntityDamager(this.entityIgniter), null,
                    this.getX(), this.getY(), this.getZ(), event.getRadius(), event.getFire(), World.a.MOB); // CraftBukkit
