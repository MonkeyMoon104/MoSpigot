...
public abstract class MobSpawnerAbstract {
        ...
        if (this.isNearPlayer(worldserver, blockposition)) {
            ...
            if (this.spawnDelay > 0) {
                ...
                for (int i = 0; i < this.spawnCount; ++i) {
                    try (ProblemReporter.j problemreporter_j = new ProblemReporter.j(this::toString, MobSpawnerAbstract.LOGGER)) {
                        ValueInput valueinput = TagValueInput.create(problemreporter_j, worldserver.registryAccess(), mobspawnerdata.getEntityToSpawn());
                        ...
                        if (worldserver.noCollision(((EntityTypes) optional.get()).getSpawnAABB(vec3d.x, vec3d.y, vec3d.z))) {
                            ...
                            Entity entity = EntityTypes.loadEntityRecursive(valueinput, worldserver, EntitySpawnReason.SPAWNER, (entity1) -> {
    ...
    public void load(@Nullable World world, BlockPosition blockposition, ValueInput valueinput) {
        this.spawnDelay = valueinput.getShortOr("Delay", (short) 20);
        valueinput.read("SpawnData", MobSpawnerData.CODEC).ifPresent((mobspawnerdata) -> {
            this.setNextSpawnData(world, blockposition, mobspawnerdata);
        });
        this.spawnPotentials = (WeightedList) valueinput.read("SpawnPotentials", MobSpawnerData.LIST_CODEC).orElseGet(() -> {
    ...
    public void save(ValueOutput valueoutput) {
        ...
        valueoutput.storeNullable("SpawnData", MobSpawnerData.CODEC, this.nextSpawnData);
        valueoutput.store("SpawnPotentials", MobSpawnerData.LIST_CODEC, this.spawnPotentials);
    }
