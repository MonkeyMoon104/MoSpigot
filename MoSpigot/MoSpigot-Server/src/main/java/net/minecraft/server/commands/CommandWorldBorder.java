...
public class CommandWorldBorder {
    ...
    private static final SimpleCommandExceptionType ERROR_TOO_BIG = new SimpleCommandExceptionType(IChatBaseComponent.translatable("commands.worldborder.set.failed.big", (double) 5.999997E7F));
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        commanddispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("worldborder").requires(net.minecraft.commands.CommandDispatcher.hasPermission(2))).then(net.minecraft.commands.CommandDispatcher.literal("add").then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("distance", DoubleArgumentType.doubleArg((double) -5.999997E7F, (double) 5.999997E7F)).executes((commandcontext) -> {
        ...
        }))))).then(net.minecraft.commands.CommandDispatcher.literal("set").then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("distance", DoubleArgumentType.doubleArg((double) -5.999997E7F, (double) 5.999997E7F)).executes((commandcontext) -> {
    ...
    private static int setDamageBuffer(CommandListenerWrapper commandlistenerwrapper, float f) throws CommandSyntaxException {
        ...
        if (worldborder.getDamageSafeZone() == (double) f) {
            ...
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.worldborder.damage.buffer.success", String.format(Locale.ROOT, "%.2f", f));
            }, true);
            return (int) f;
        }
    }
    ...
    private static int setDamageAmount(CommandListenerWrapper commandlistenerwrapper, float f) throws CommandSyntaxException {
        ...
        if (worldborder.getDamagePerBlock() == (double) f) {
            ...
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.worldborder.damage.amount.success", String.format(Locale.ROOT, "%.2f", f));
            }, true);
            return (int) f;
        }
    }
    ...
    private static int setWarningTime(CommandListenerWrapper commandlistenerwrapper, int i) throws CommandSyntaxException {
        ...
        if (worldborder.getWarningTime() == i) {
            ...
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.worldborder.warning.time.success", i);
            }, true);
            return i;
        }
    }
    ...
    private static int setWarningDistance(CommandListenerWrapper commandlistenerwrapper, int i) throws CommandSyntaxException {
        ...
        if (worldborder.getWarningBlocks() == i) {
            ...
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.worldborder.warning.distance.success", i);
            }, true);
            return i;
        }
    }
    ...
    private static int setCenter(CommandListenerWrapper commandlistenerwrapper, Vec2F vec2f) throws CommandSyntaxException {
        ...
        if (worldborder.getCenterX() == (double) vec2f.x && worldborder.getCenterZ() == (double) vec2f.y) {
            ...
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", vec2f.x), String.format(Locale.ROOT, "%.2f", vec2f.y));
            }, true);
            return 0;
        } else {
    ...
    private static int setSize(CommandListenerWrapper commandlistenerwrapper, double d0, long i) throws CommandSyntaxException {
        ...
        if (d1 == d0) {
            ...
            if (i > 0L) {
                ...
                if (d0 > d1) {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", d0), Long.toString(i / 1000L));
                    }, true);
                } else {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", d0), Long.toString(i / 1000L));
                    }, true);
                }
            } else {
                worldborder.setSize(d0);
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.worldborder.set.immediate", String.format(Locale.ROOT, "%.1f", d0));
                }, true);
            }
