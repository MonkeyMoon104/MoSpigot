    ...
    public void handleStatusRequest(PacketStatusInStart packetstatusinstart) {
        if (this.hasRequestedStatus) {
            ...
            if ( !server.hidesOnlinePlayers() && !profiles.isEmpty() )
            {
                ...
                profiles = profiles.subList( 0, Math.min( profiles.size(), org.mospigot.config.MoSpigotConfig.playerSample ) ); // Cap the sample to n (or less) displayed players, ie: Vanilla behaviour
            }
            // Spigot End
