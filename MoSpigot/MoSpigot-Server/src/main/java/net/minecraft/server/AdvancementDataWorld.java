...
public class AdvancementDataWorld extends ResourceDataJson<Advancement> {
    ...
    protected void apply(Map<MinecraftKey, Advancement> map, IResourceManager iresourcemanager, GameProfilerFiller gameprofilerfiller) {
        ...
        map.forEach((minecraftkey, advancement) -> {
            // Spigot start
            if (org.mospigot.config.MoSpigotConfig.disabledAdvancements != null && (org.mospigot.config.MoSpigotConfig.disabledAdvancements.contains("*") || org.mospigot.config.MoSpigotConfig.disabledAdvancements.contains(minecraftkey.toString()) || org.mospigot.config.MoSpigotConfig.disabledAdvancements.contains(minecraftkey.getNamespace()))) {
                return;
            }
            // Spigot end
