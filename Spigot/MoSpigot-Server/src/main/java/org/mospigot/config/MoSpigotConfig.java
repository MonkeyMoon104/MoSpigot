...
import org.mospigot.Metrics;
import org.mospigot.commands.*;
import org.mospigot.WatchdogThread;

public class MoSpigotConfig
{
    ...
    public static void init(File configFile)
    {
        ...
        commands.put( "mospigot", new MoSpigotCommand( "mospigot" ) );

        version = getInt( "config-version", 12 );
        set( "config-version", 12 );
        readConfig( MoSpigotConfig.class, null );
    }
    ...
    public static void registerCommands()
    {
        ...
        {
            MinecraftServer.getServer().server.getCommandMap().register( entry.getKey(), "MoSpigot", entry.getValue() );
        }
    ...
    private static boolean getBooleanSafe(String path, boolean def)
    {
        config.addDefault(path, def);
    ...
    private static void geoCommand() {
        commands.put( "geo", new GeoCommand("geo"));
    }

    private static void authCommand() {
        commands.put( "auth", new AuthCommand("auth"));
    }
    ...
    private static void discordWebhook() {
        discordWebhookEnabled = getBooleanSafe("settings.discord.enabled", false);
        discordWebhookUrl = getString("settings.discord.webhook-url", "");
    }
