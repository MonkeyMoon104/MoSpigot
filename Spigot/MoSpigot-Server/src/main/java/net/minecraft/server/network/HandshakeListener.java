        ...
        switch (packethandshakinginsetprotocol.intention()) {
            ...
            case TRANSFER:
                if (!this.server.acceptsTransfers()) {
                    ...
                    IChatBaseComponent ichatbasecomponent = IChatBaseComponent.translatable("multiplayer.disconnect.transfers_disabled");
    ...
    private void beginLogin(PacketHandshakingInSetProtocol packethandshakinginsetprotocol, boolean flag) {
        ...
        if (packethandshakinginsetprotocol.protocolVersion() != SharedConstants.getCurrentVersion().protocolVersion()) {
            ...
            if (packethandshakinginsetprotocol.protocolVersion() < SharedConstants.getCurrentVersion().protocolVersion()) { // Spigot - SPIGOT-7546: Handle version check correctly for outdated client message
                ichatbasecomponent = IChatBaseComponent.literal( java.text.MessageFormat.format( org.mospigot.config.MoSpigotConfig.outdatedClientMessage.replaceAll("'", "''"), SharedConstants.getCurrentVersion().name() ) ); // Spigot
            } else {
                ichatbasecomponent = IChatBaseComponent.literal( java.text.MessageFormat.format( org.mospigot.config.MoSpigotConfig.outdatedServerMessage.replaceAll("'", "''"), SharedConstants.getCurrentVersion().name() ) ); // Spigot
            }
            ...
            if (org.mospigot.config.MoSpigotConfig.bungee) {
