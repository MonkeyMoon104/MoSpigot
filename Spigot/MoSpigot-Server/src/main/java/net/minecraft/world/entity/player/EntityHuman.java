...
public abstract class EntityHuman extends EntityLiving {
    ...
    protected void addAdditionalSaveData(ValueOutput valueoutput) {
        ...
        GameProfileSerializer.addCurrentDataVersion(valueoutput);
        ...
        valueoutput.store("abilities", PlayerAbilities.a.CODEC, this.abilities.pack());
        this.enderChestInventory.storeAsSlots(valueoutput.list("EnderItems", ItemStackWithSlot.CODEC));
        if (!this.getShoulderEntityLeft().isEmpty()) {
            valueoutput.store("ShoulderEntityLeft", NBTTagCompound.CODEC, this.getShoulderEntityLeft());
        }

        if (!this.getShoulderEntityRight().isEmpty()) {
            valueoutput.store("ShoulderEntityRight", NBTTagCompound.CODEC, this.getShoulderEntityRight());
        }

        this.lastDeathLocation.ifPresent((globalpos) -> {
            valueoutput.store("LastDeathLocation", GlobalPos.CODEC, globalpos);
        });
        valueoutput.storeNullable("current_explosion_impact_pos", Vec3D.CODEC, this.currentImpulseImpactPos);
