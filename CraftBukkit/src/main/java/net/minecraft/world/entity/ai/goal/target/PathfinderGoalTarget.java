...
public abstract class PathfinderGoalTarget extends PathfinderGoal {
    ...
    public boolean canContinueToUse() {
        ...
        if (entityliving == null) {
            ...
            ScoreboardTeamBase scoreboardteambase = this.mob.getTeam();
            ScoreboardTeamBase scoreboardteambase1 = entityliving.getTeam();
    ...
    protected boolean canAttack(@Nullable EntityLiving entityliving, PathfinderTargetCondition pathfindertargetcondition) {
        if (entityliving == null) {
            return false;
        } else if (!pathfindertargetcondition.test(getServerLevel((Entity) this.mob), this.mob, entityliving)) {
            return false;
        } else if (!this.mob.isWithinHome(entityliving.blockPosition())) {
            return false;
        } else {
            if (this.mustReach) {
    ...
    private boolean canReach(EntityLiving entityliving) {
        ...
        PathEntity pathentity = this.mob.getNavigation().createPath(entityliving, 0);
