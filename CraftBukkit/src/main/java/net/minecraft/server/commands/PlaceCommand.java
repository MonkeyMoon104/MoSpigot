...
public class PlaceCommand {
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher) {
        commanddispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("place").requires(net.minecraft.commands.CommandDispatcher.hasPermission(2))).then(net.minecraft.commands.CommandDispatcher.literal("feature").then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("feature", ResourceKeyArgument.key(Registries.CONFIGURED_FEATURE)).executes((commandcontext) -> {
            ...
            return placeTemplate((CommandListenerWrapper) commandcontext.getSource(), ArgumentMinecraftKeyRegistered.getId(commandcontext, "template"), BlockPosition.containing(((CommandListenerWrapper) commandcontext.getSource()).getPosition()), EnumBlockRotation.NONE, EnumBlockMirror.NONE, 1.0F, 0, false);
            ...
            return placeTemplate((CommandListenerWrapper) commandcontext.getSource(), ArgumentMinecraftKeyRegistered.getId(commandcontext, "template"), ArgumentPosition.getLoadedBlockPos(commandcontext, "pos"), EnumBlockRotation.NONE, EnumBlockMirror.NONE, 1.0F, 0, false);
            ...
            return placeTemplate((CommandListenerWrapper) commandcontext.getSource(), ArgumentMinecraftKeyRegistered.getId(commandcontext, "template"), ArgumentPosition.getLoadedBlockPos(commandcontext, "pos"), TemplateRotationArgument.getRotation(commandcontext, "rotation"), EnumBlockMirror.NONE, 1.0F, 0, false);
            ...
            return placeTemplate((CommandListenerWrapper) commandcontext.getSource(), ArgumentMinecraftKeyRegistered.getId(commandcontext, "template"), ArgumentPosition.getLoadedBlockPos(commandcontext, "pos"), TemplateRotationArgument.getRotation(commandcontext, "rotation"), TemplateMirrorArgument.getMirror(commandcontext, "mirror"), 1.0F, 0, false);
    ...
    public static int placeStructure(CommandListenerWrapper commandlistenerwrapper, Holder.c<Structure> holder_c, BlockPosition blockposition) throws CommandSyntaxException {
        ...
        StructureStart structurestart = structure.generate(holder_c, worldserver.dimension(), commandlistenerwrapper.registryAccess(), chunkgenerator, chunkgenerator.getBiomeSource(), worldserver.getChunkSource().randomState(), worldserver.getStructureManager(), worldserver.getSeed(), new ChunkCoordIntPair(blockposition), 0, worldserver, (holder) -> {
            return true;
        });
    ...
    public static int placeTemplate(CommandListenerWrapper commandlistenerwrapper, MinecraftKey minecraftkey, BlockPosition blockposition, EnumBlockRotation enumblockrotation, EnumBlockMirror enumblockmirror, float f, int i, boolean flag) throws CommandSyntaxException {
        WorldServer worldserver = commandlistenerwrapper.getLevel();
        ...
        if (optional.isEmpty()) {
            ...
            DefinedStructureInfo definedstructureinfo = (new DefinedStructureInfo()).setMirror(enumblockmirror).setRotation(enumblockrotation).setKnownShape(flag);
            ...
            boolean flag1 = definedstructure.placeInWorld(worldserver, blockposition, blockposition, definedstructureinfo, TileEntityStructure.createRandom((long) i), 2 | (flag ? 816 : 0));
