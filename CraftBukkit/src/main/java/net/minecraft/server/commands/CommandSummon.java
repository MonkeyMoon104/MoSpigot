...
public class CommandSummon {
    ...
    public static void register(CommandDispatcher<CommandListenerWrapper> commanddispatcher, CommandBuildContext commandbuildcontext) {
        commanddispatcher.register((LiteralArgumentBuilder) ((LiteralArgumentBuilder) net.minecraft.commands.CommandDispatcher.literal("summon").requires(net.minecraft.commands.CommandDispatcher.hasPermission(2))).then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("entity", ResourceArgument.resource(commandbuildcontext, Registries.ENTITY_TYPE)).suggests(CompletionProviders.cast(CompletionProviders.SUMMONABLE_ENTITIES)).executes((commandcontext) -> {
    ...
    public static Entity createEntity(CommandListenerWrapper commandlistenerwrapper, Holder.c<EntityTypes<?>> holder_c, Vec3D vec3d, NBTTagCompound nbttagcompound, boolean flag) throws CommandSyntaxException {
        ...
        if (!World.isInSpawnableBounds(blockposition)) {
            ...
            Entity entity = EntityTypes.loadEntityRecursive(nbttagcompound1, worldserver, EntitySpawnReason.COMMAND, (entity1) -> {
